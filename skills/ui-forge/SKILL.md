---
name: ui-forge
description: Convert mixed brand references, screenshots, mobile wireframes, assets, copy, annotations, and Figma links into an evidence-based brand system and distinct mobile UI directions, including reference-derived text-media relationships and role-correct original assets; rebuild selected directions as editable Figma frames, then publish a node-mapped screenshot Visual Review for annotation-driven revisions. Use when Codex needs to extract brand DNA, analyze how reference UI pairs copy with icons or imagery, enrich sparse wireframes without changing their product structure, design iOS or Android screens, generate product or campaign UI options, separate native, transparent, and composite raster layers, create Auto Layout screen sets, review Figma screens, or apply screenshot feedback back to precise nodes.
---
...
Before showing A, B, and C or entering `AWAITING_PRIMARY_OPTION_SELECTION`, create `wireframe-content-lock.json` and `reference-media-plan.json` from the bundled templates, then run `scripts/validate_preview_gates.py` against all three layer specs. The validator must return `pass` and write `preview-gates.json`.
...
3. **Reference media relationships:** analyze component anatomy, text-media pairing, asset type, position, and visual share from reference evidence. Map those patterns to existing locked modules. A text-only wireframe does not erase reference-supported icons or imagery; add visual layers inside the existing module without adding product copy, actions, or information architecture. A text-and-color-block option remains valid when the media omission is deliberate and recorded.
4. **Raster role:** classify each raster before checking its background. Isolated objects use `transparent_required` and must have real alpha, transparent coverage, and clear outer borders. Original integrated scenes, complex visual surfaces, and full-bleed backgrounds use `opaque_composite_expected`; keep copy and controls as native overlays. Use `embedded_background_authorized` only for an exact user-authorized asset whose background is intentionally retained.
...
4. **Visual Review integrity:** after every Figma write, generate the complete screenshot gallery only with the bundled `scripts/generate_visual_review.py`, then run `scripts/validate_visual_review.py`. Never hand-author, copy, restyle, or patch the generated HTML. Template drift, stale manifests, missing screens, non-natural screenshot dimensions, or validator failure blocks completion.

Do not claim Figma delivery complete, generate a final handoff, or advance to `AWAITING_ADDITIONAL_OPTION_SELECTION` until the Figma delivery validator and Visual Review validator both return `pass`. Record evidence and exceptions in `delivery-gates.json`; exceptions cannot reduce the 12-asset minimum or exempt structural containers and eligible token properties.
...
Read [references/reference-media-composition.md](references/reference-media-composition.md). Create `reference-media-plan.json` from the bundled template. Extract how reference modules pair icons, people, products, 3D, illustration, composite scenes, textures, or native visuals with titles and body copy. Record position and visual-share bands, then map each pattern to a locked `structure_id`. Do not infer “no media” merely because a sparse wireframe omitted visual detail.
...
Do not place reference imagery directly into any option preview. Generate new original complex assets from the extracted design DNA and subject category. Do not reproduce reference people, devices, logos, screenshots, mascots, props, or distinctive compositions. Generate isolated objects with a real transparent background. Generate an original complete opaque image when the intended role is a composite scene, full-bleed background, or complex visual surface whose elements cannot be faithfully separated. Record `asset_role`, target module, composition purpose, permission, origin, and `background_policy` in `asset-provenance.json`.
...
After rendering, run `scripts/validate_preview_gates.py` with the content lock, reference media plan, provenance manifest, project root, and all three layer specs. Do not present options until it writes a passing `preview-gates.json`.
...
Prefer asset-before-preview: generate original complex assets independently with role-correct transparent or opaque backgrounds and known bounds, then compose previews from the same asset files and layer specification that will be written to Figma. Never use generated screenshot text as final text. Rebuild text natively. Do not extract or reuse a reference-only asset merely because it is already isolated or transparent.
...
Immediately validate the generated result:
...
Treat `index.html` as a generated artifact. Do not edit it directly or substitute another review template. If the validator fails, repair the manifest or screenshots and rerun the bundled generator; never patch the HTML to make it appear correct.
...
- Require `validate_visual_review.py` to pass after generation; never report a hand-authored or modified gallery as UIForge Visual Review.
...
- Never require alpha from a role-correct generated composite scene, surface texture, or full-bleed background.
- Never default a sparse wireframe to text and color blocks before analyzing reference text-media pairing, asset type, position, and visual share.
...
Complete preview generation only when inputs are classified, Brand DNA is summarized, the wireframe is interpreted, exact wireframe copy and required structure are locked, reference text-media relationships are mapped to existing modules with explicit A/B/C decisions, isolated rasters pass alpha validation, generated composites use role-correct opaque policies, a valid material-style profile controls original asset generation, library and icon decisions are recorded, and `validate_preview_gates.py` writes a passing `preview-gates.json` for A, B, and C. Only then present the three labeled home options and wait for one primary selection. Complete primary Figma delivery only when the selected direction is extended across the remaining wireframes; every accepted one of at least 12 meaningful reusable candidates matches the approved material-style profile and is present and labeled in the Figma asset library; every managed screen root and structural container passes Auto Layout validation; every required Brand DNA token is mapped to a variable and every eligible managed-screen property is bound; `validate_delivery_gates.py` returns `pass`; sourced icons remain editable vectors; selected component and chart patterns remain themed and editable; the approved home preview is reproduced from the same layers; visual QA is recorded; and an up-to-date Visual Review containing every managed screen is generated by the bundled script and returns `pass` from `validate_visual_review.py`. Then wait for screenshot annotations or the optional additional-option decision.
