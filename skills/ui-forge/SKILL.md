...
2. **Auto Layout coverage:** every managed screen root must use Auto Layout. Every structural container inside a screen—including sections, stacks, rows, cards, lists, action groups, and navigation—must use Auto Layout. Only vectors, raster/image nodes, simple leaf shapes, and explicitly documented decorative or absolute overlays may be exceptions. Any undocumented structural `layoutMode: NONE` node fails the gate.
...
Read [references/same-source-pipeline.md](references/same-source-pipeline.md). Create and validate one `*.layers.json` file for each home direction from `assets/layer-spec-template.json`. Render the three previews with `scripts/render_layer_spec.py`. The renderer must use the same asset files, bounds, crop, rotation, opacity, and z-order that Figma will use.
