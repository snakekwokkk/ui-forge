...
4. Create `option-<id>-home.layers.json` from `assets/layer-spec-template.json`.
...
9. Build Figma nodes from the same ordered layer list and asset files.
...
- Preserve array order as the default z-order; use `z_index` for validation and explicit ordering.
