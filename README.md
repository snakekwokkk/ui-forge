...
- 区分透明独立素材与完整复合图片：独立物件检查真实 Alpha；场景、卡面纹理和全幅背景允许使用角色正确的原创不透明图片。
...
- Requires real alpha for isolated rasters while allowing role-correct original composite scenes, card textures, and full-bleed backgrounds.
