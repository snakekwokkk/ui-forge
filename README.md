...
- 检查独立位图素材的真实 Alpha 透明度，拒绝白底、浅色底或摄影棚底的矩形贴图。
...
2. UIForge 自动分类输入、提取 Brand DNA，并锁定线框图内容与结构。
3. 生成三个具有明显差异的首页方向，并通过文案、结构和透明素材门禁：
   - A — Product Clear
   - B — Brand Balanced
   - C — Campaign Bold
4. 选择一个主要方向。
...
不要把密码、API Key、私人客户资料或公司机密提交到公开仓库。
...
- Rejects isolated raster assets that lack real alpha transparency or retain a baked rectangular background.
...
- Enforces preview gates for wireframe fidelity and raster transparency, plus delivery gates for asset placement, Auto Layout coverage, variable binding, and visual consistency.
