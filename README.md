...
- Visual Review 只能由插件自带生成器输出，并通过模板完整性、页面数量和截图自然尺寸校验；禁止另建或手改评审模板。
...
7. 完成视觉检查，使用官方生成器创建 Visual Review，并通过独立验证器后，才可通过截图标注继续修改。
...
- Requires the bundled generator and a fail-closed validator for Visual Review template integrity, complete screen coverage, and exact natural screenshot dimensions.
